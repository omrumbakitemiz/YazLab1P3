...
namespace yazlab1p3.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpGet]
        public IActionResult SemantikAnaliz()
        {
            return View();
        }

        [HttpPost]
        public IActionResult SemantikAnaliz(UrlKeyword urlKeyword)
        {
            if (string.IsNullOrEmpty(urlKeyword.Urls[0]) || string.IsNullOrEmpty(urlKeyword.Keywords[0]))
            {
                return RedirectToAction("Error");
            ...
            var sonuc = Utilities.SemantikAnaliz(urlList, model[0].AnahtarKelimeler);
            var data = sonuc.Select(p => p.AnahtarKelimeGecmeSayisi).ToArray();
